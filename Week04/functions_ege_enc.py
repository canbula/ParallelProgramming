...
def fn_w_counter() -> tuple[int, dict]:
    ...
    It keeps track of:
      ...
      - a dictionary (caller_info) where the key is the caller name and the value 
        is the number of times that caller has invoked the function.
    
    :return: A tuple containing:
      - an integer (total number of calls)
      - a dictionary with caller names as keys and the number of times 
        they've called the function as values.
    """
    ...
    return int(fn_w_counter.counter), dict(fn_w_counter.caller_info)
