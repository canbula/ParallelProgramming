...
class Philosopher(threading.Thread):
    ...
    def process(self):
        # Check if both forks are picked up successfully
        if self.left_fork.picked_up and self.right_fork.picked_up:
            self.spaghetti -= 1
            self.eating = True
            time.sleep(5 + random.random() * 5)
            self.eating = False
        else:
            # If the second fork is occupied, release the first fork
