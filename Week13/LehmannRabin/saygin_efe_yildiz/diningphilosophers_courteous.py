...
class Philosopher(threading.Thread):
    def __init__(self, index: int, left_fork: Fork, right_fork: Fork, spaghetti: int):
        ...
        self.trying: bool = False
        self.left_philosopher: Optional[Philosopher] = None
        self.right_philosopher: Optional[Philosopher] = None
...
def animated_table(philosophers: list[Philosopher], forks: list[Fork], m: int):
    ...
    philosopher_circles: list[mpatches.Circle] = [
        mpatches.Circle((0, 0), 0.2, color="black") for _ in range(len(philosophers))
    ]
    philosopher_texts: list[mtext.Text] = [
        mtext.Text(
            0,
            0,
    ...
    fork_lines: list[mlines.Line2D] = [
        mlines.Line2D((0, 0), (0, 0), color="black") for _ in range(len(forks))
    ]
    fork_texts: list[mtext.Text] = [
        mtext.Text(
            0,
            0,
            str(fork.index),
    ...
    ani = animation.FuncAnimation(
        fig, update, frames=range(100000), interval=10, blit=False #type: ignore
    )
    plt.show()
...
def main() -> None:
    ...
    for i in range(len(philosophers)):
        philosophers[i].left_philosopher = philosophers[(i - 1) % n]
        philosophers[i].right_philosopher = philosophers[(i + 1) % n]
