...
class Fork:
    ...
    #     if self.lock.acquire(timeout=12):
    #         self.owner = owner
    #         self.picked_up = True
...
class Philosopher(threading.Thread):
    ...
    def eat(self):
        left_fork_ = self.left_fork.lock.acquire(timeout=3)
        if left_fork_:
            self.left_fork.owner = self.index
            self.left_fork.picked_up = True
            time.sleep(5 + random.random() * 5)
            right_fork_ = self.right_fork.lock.acquire(timeout=3)
            if not right_fork_:
                print(f"--- Philosopher#{self.index} timed out acquiring the right fork. Releasing the left fork ---")
                self.left_fork.lock.release()
                self.left_fork.picked_up = False
                self.left_fork.owner = -1
            else:
                self.right_fork.owner = self.index
                self.right_fork.picked_up = True
                print(f"--- Philosopher#{self.index} acquired both forks and is eating right now ---")
                self.spaghetti -= 1
                self.eating = True
                time.sleep(5 + random.random() * 5)
                print(f"--- Philosopher#{self.index} is done with eating ---")
                self.eating = False
                self.left_fork.lock.release()
                self.left_fork.picked_up = False
                self.left_fork.owner = -1
                self.right_fork.lock.release()
                self.right_fork.picked_up = False
                self.right_fork.owner = -1

        else:
            print(f"--- Philosopher#{self.index} ran out of time to acquire the left fork ---")   

        # with self.left_fork(self.index) as left_fork_:
        #     time.sleep(5 + random.random() * 5)
        #     with self.right_fork(self.index) as right_fork_:
        #         self.spaghetti -= 1
        #         self.eating = True
        #         time.sleep(5 + random.random() * 5)
        #         self.eating = False
...
def animated_table(philosophers: list[Philosopher], forks: list[Fork], m: int):
    ...
    ani = animation.FuncAnimation(
        fig, update, frames=range(100000), interval=1, blit=False
    )
    plt.show()
