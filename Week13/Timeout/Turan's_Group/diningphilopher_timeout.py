...
class Fork:
    def __init__(self, index: int, timeout: int = 5):  # burada timeout attribute ekliyoruz; java da constructor yapısnın aynısı
        self.index: int = index
        self.lock: threading.Lock = threading.Lock()
        self.picked_up: bool = False
        self.owner: int = -1
        self.timeout: int = timeout  # attribute burada da kullanışlı bir atama yapıyoruz
...
class Philosopher(threading.Thread):
    def __init__(self, index: int, left_fork: Fork, right_fork: Fork, spaghetti: int, timeout: int = 5):  # burada da yine constructura ekliyoruz 
        super().__init__()
        self.index: int = index
        ...
        self.timeout: int = timeout # aynısı

    def run(self):
        while self.spaghetti > 0:
            self.think()
            self.eat_timeout()  # run fonksiyonu bizim temel fonksiyonumuz time_out foksiyonunu da buraya ekliyoruz. 
    ...
    def eat_timeout(self):  
        with self.left_fork(self.index) as left, self.right_fork(self.index) as right:     
            if left and right:  
                self.spaghetti -= 1 
                self.eating = True  
                time.sleep(5 + random.random() * 5)
