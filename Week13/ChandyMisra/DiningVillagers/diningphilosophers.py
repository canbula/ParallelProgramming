...
class Philosopher(threading.Thread):
    def __init__(self, index: int, left_fork: Fork, right_fork: Fork, spaghetti: int, forks: list[Fork]):
        super().__init__()
        self.index: int = index
    ...
    def request_forks(self):
        # Chandy/Misra algorithm: send fork requests to neighbors
        for fork in self.forks:
            if fork.index == self.index:
                continue
            with fork(self.index):
                pass
    ...
    def release_forks(self):
        # Chandy/Misra algorithm: release forks and notify neighbors
        for fork in self.forks:
            if fork.index == self.index:
                continue
            with fork(self.index):
