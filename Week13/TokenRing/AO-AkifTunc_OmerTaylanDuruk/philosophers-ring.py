...
class Philosopher(threading.Thread):
    def __init__(self, index: int, left_fork: Fork, right_fork: Fork, spaghetti: int, token_queue: Queue):
        super().__init__()
        self.index: int = index
        ...
        self.token_queue: Queue = token_queue
    ...
    def request_token(self):
        self.token_queue.put(self.index)
...
def main() -> None:
    ...
    philosophers: list[Philosopher] = [
        Philosopher(i, forks[i], forks[(i + 1) % n], m, token_queue) for i in range(n)
    ]
