...
class Philosopher(threading.Thread):
    def __init__(self, index: int, left_fork: Fork, right_fork: Fork, spaghetti: int, token: threading.Lock):
        super().__init__()
        self.index: int = index
        ...
        self.token: threading.Lock = token
    ...
    def eat(self):
     with self.token:
        first_fork, second_fork = sorted([self.left_fork, self.right_fork], key=lambda x: x.index)
