...
def test_names():
    for f in files:
        ...
        assert "my_complex" in dir(eval(f[:-3])), "my_complex is not defined in " + f[:-3]

def test_types():
    for f in files:
        assert isinstance(eval(f[:-3]).my_int, int), "my_int is not an int in " + f[:-3]
        assert isinstance(eval(f[:-3]).my_float, float), "my_float is not a float in " + f[:-3]
        assert isinstance(eval(f[:-3]).my_bool, bool), "my_bool is not a bool in " + f[:-3]
        assert isinstance(eval(f[:-3]).my_complex, complex), "my_complex is not a complex in " + f[:-3]
